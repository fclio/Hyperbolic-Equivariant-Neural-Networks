...
class GroupLorentzFullyConnected(nn.Module):
    def __init__(
        ...
        # extract the weight for x_space:
        # using x.narrow to 
        weight_space = self.weight.weight.narrow( 1, self.weight.weight.shape[-1] - 1)
    ...
    def forward(self, x):
        ...
        # i can pass the weight manually like F.conv2d
        # Apply linear transformation with custom weight !!!!
        # x = F.linear(x_patches, weight_reshaped)  # (128, num_patches, 512)
