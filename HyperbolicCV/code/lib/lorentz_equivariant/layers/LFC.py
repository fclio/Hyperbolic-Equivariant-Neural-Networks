...
def combine_weight_kernel(tw_space, w_time_output, w_time_input, device):
    ...
    tw_linear = torch.cat([w_time_output.to(device), tw_space_linear.to(device)], dim=0)
...
class GroupLorentzFullyConnected(nn.Module):
    def __init__(
        ...
        self.weight_time_output = Parameter(torch.Tensor(
            1, (self.in_channels-1) * self.input_stabilizer_size * self.kernel_size[0] * self.kernel_size[1]), requires_grad=True)
