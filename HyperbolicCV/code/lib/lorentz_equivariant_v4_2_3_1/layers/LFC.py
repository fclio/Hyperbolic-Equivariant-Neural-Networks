...
def combine_weight_kernel(tw_space, w_time_output, w_time_input, device):
    ...
    w_time_output = w_time_output.unsqueeze(2).repeat(1, 1, input_stabilizer_size, 1)
    ...
    w_time_input = w_time_input.unsqueeze(1).repeat(1, input_stabilizer_size, 1)
...
class GroupLorentzFullyConnected(nn.Module):
    def __init__(
        ...
        self.weight_time_output = Parameter(torch.Tensor(
            1, 1, (self.in_channels - 1) * self.kernel_size[0] * self.kernel_size[1]), requires_grad=True)
    ...
    def forward(self, x):
        ...
        group_size = out_c // self.output_stabilizer_size
        x = x.view(batch_size,  num_patches, self.output_stabilizer_size, group_size)  # shape: [B, n, G, F]
    
        # print("x 2", x.shape)
        x = x.permute(2,0,1,3)  # shape: [G, B, n, F]
        # print("x 3", x.shape)

        out = [self.normalize_time(gx) for gx in x]
        x_final = torch.stack(out, dim=0)  # shape: [G, B, n, F]
        # print("x 4", x_final.shape)

        return x_final.permute(1, 0, 2, 3)     # shape: [B, G, n, F]
